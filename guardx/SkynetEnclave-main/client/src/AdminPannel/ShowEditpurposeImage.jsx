...
function ShowEditpurposeImage(props) {
  ...
  return (
    <div>
      ...
      >
        <Box sx={style}>
          {props.data.purposeIcon ? (
            <div className="edit_Entry_image_modal">
              <img
                src={`/${props.data.purposeIcon.replace("public/", "")}`}
                alt=""
              />
            </div>
          ) : (
            <>
              <h6>
                {props.data.purpose ? (
                  (() => {
                    let words = props.data.purpose.split(" ");
...
export default ShowEditpurposeImage;
